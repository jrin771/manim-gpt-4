...
class Hypercube(ThreeDScene):
    def construct(self):
        ...
        for angle in np.linspace(0, 2 * np.pi, 5):
            ...
            edges = []
            for i, coord1 in enumerate(rotated_coords):
                for j, coord2 in enumerate(rotated_coords):
                    if np.sum(np.abs(coord1 - coord2)) == 2:
                        edge = Line(dots[i].get_center(), dots[j].get_center(), stroke_width=0.2 * 10)
                        edges.append(edge)
